...
function Header({ current }: { current: PageKey }) {
  ...
  return <header className="site-header"><div className="nav-wrap">
    <Link className="brand" href="/" aria-label="Launchpad Web Solutions home"><span className="brand-mark">↗</span>Launchpad</Link>
...
function Footer() {
  return <footer className="footer"><div className="footer-top">
    <div><Link className="brand brand-light" href="/"><span className="brand-mark">↗</span>Launchpad</Link><p>Websites built to move Australian business forward.</p></div>
    <div><p className="footer-label">Explore</p>{nav.slice(1).map(([, label, href]) => <Link key={href} href={href}>{label}</Link>)}</div>
    <div><p className="footer-label">Get in touch</p><a href="mailto:hello@launchpadweb.com.au">hello@launchpadweb.com.au</a><a href="tel:1300123456">1300 123 456</a><p>Sydney, NSW, Australia</p></div>
...
function OrbitVisual() {
  return <div className="orbit-visual" aria-label="A responsive website interface launching into orbit">
    ...
    <div className="launch-chip"><span>↗</span> Ready to launch</div>
  </div>;
}
